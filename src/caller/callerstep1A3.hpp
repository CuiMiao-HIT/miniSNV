...
void create_candidate_code(char *DUPBED_PATH, char *BAM_PATH, char *INDEX_PATH, int give_chrid, uint64_t readbam_start, uint64_t readbam_end, std :: map<uint32_t , PosInfo> & candidata_p1);
...
// void create_highvrt_p2vrt(char *BED_PATH, std :: map<uint32_t , PosInfo> candidata_p1, std :: vector <snvVRT> &ResultSnv, ofstream &LOWSNV_Info_F);
