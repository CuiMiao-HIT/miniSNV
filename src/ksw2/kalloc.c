...
static size_t *morecore(kmem_t *km, size_t nu)
{
	...
	*up = rnu; /* the size of the current block, and in this case the block is the same as the new core */
	kfree_(km, up + 1); /* initialize the new "core" */
	return km->loop_head;
}

void km_destroy_(void *_km)
{
...
void *krealloc_(void *_km, void *ap, size_t n_bytes)
{
	kmem_t *km = (kmem_t*)_km;
	size_t n_units, *p, *q;

	if (n_bytes == 0) {
		kfree_(km, ap); return 0;
	}
	if (km == 0) return realloc(ap, n_bytes);
	if (!ap) return kmalloc_(km, n_bytes);
	n_units = 1 + (n_bytes + sizeof(size_t) - 1) / sizeof(size_t);
	p = (size_t*)ap - 1;
	if (*p >= n_units) return ap; /* TODO: this prevents shrinking */
	q = (size_t*)kmalloc_(km, n_bytes);
	memcpy(q, ap, (*p - 1) * sizeof(size_t));
	kfree_(km, ap);
	return q;
}

void *kmalloc_(void *_km, size_t n_bytes)
{
	kmem_t *km = (kmem_t*)_km;
	size_t n_units, *p, *q;
...
void *kcalloc_(void *_km, size_t count, size_t size)
{
	kmem_t *km = (kmem_t*)_km;
	void *p;
	if (size == 0 || count == 0) return 0;
	if (km == 0) return calloc(count, size);
	p = kmalloc_(km, count * size);
	memset(p, 0, count * size);
	return p;
}

void km_stat_(const void *_km)
{
	kmem_t *km = (kmem_t*)_km;
